...
void permutations(int total_index, std::vector<char> &areSol, int current_index, vector<bool> &used, vector<char> &candidateSolution)
{
    ...
    if (current_index < total_index)
    {
        ...
        for (int i = 0; i < total_index; i++) // possible choice for n_th depth = total_index - n
        {
            ...
            if (used[i] == false)
            {
                ...
                permutations(total_index, areSol, current_index + 1, used, candidateSolution);
                used[i] = false;
            }
        }
    }
...
int main()
{
    ...
    vector<char> candidateSolution({'L', 'I', 'S', 'T', 'E', 'N'});
    ...
    permutations(total_index, areSol, 0, used, candidateSolution);
