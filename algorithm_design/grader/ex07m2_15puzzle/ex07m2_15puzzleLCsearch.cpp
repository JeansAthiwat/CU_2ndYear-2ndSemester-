...
int lcs_solve(vector<vector<int>> &startingBoard, int startingR, int startingC)
{
    set<Board> recognizedBoard;
    priority_queue<Board, vector<Board>, HeuristicComparer> states;
    Board starting_board(startingBoard, startingR, startingC);
    states.push(starting_board);
...
int main()
{
    ...
    int min_moves = lcs_solve(tmpBoard, emptyR, emptyC);
    cout << min_moves;
    return 0;
}
